...
seurat_obj@meta.data$cell_type_original <- "Other"
seurat_obj@meta.data$cell_type_original[rownames(seurat_obj@meta.data) %in% CD8T_cellID$Cell.Name] <- "CD8+ T-cells"




# save raw seurat object with all genes and all cells
saveRDS(object = seurat_obj, file = paste0(data_dir, "seu_",sampleType_name,"_raw.rds")) 


merged_seu_subset <- subset(seurat_obj, subset = cell_type_original %in% c("CD8+ T-cells"))


# filter genes to only focus on protein coding genes (Safe-Feldman paper list)
protein_coding_genes=read.csv(file = paste0(data_dir,"Feldman_protein_coding_genes.csv"), header=T,colClasses = "character")[,1]
genes <- intersect(protein_coding_genes, row.names(merged_seu_subset))
merged_seu_subset <- merged_seu_subset[genes, ]

# filter genes to only focus on protein coding genes (PENCIL paper method)
protein_coding_gene_file = paste0(data_dir,'/gene_with_protein_product.txt')
protein_coding_genes=read.delim(protein_coding_gene_file, header=F,colClasses = "character")[,2]
genes <- intersect(protein_coding_genes, row.names(merged_seu_subset))
merged_seu_subset <- merged_seu_subset[genes, ]
...
#
merged_seu_subset <- RunPCA(object=merged_seu_subset, features=VariableFeatures(merged_seu_subset))
...
exp_data_scaled = merged_seu@assays[["RNA"]]@scale.data[VariableFeatures(merged_seu),]
write.csv(exp_data_scaled, file=paste0(data_dir,"exp_data_mvg2000_scale_self.csv"))

logTPM_counts = merged_seu@assays[["RNA"]]@counts[VariableFeatures(merged_seu),]
write.csv(logTPM_counts, file=paste0(data_dir, 'exp_data_mvg2000_log2TPM_self.csv'))
...
#plot umap
DimPlot(object=merged_seu_subset, reduction='umap', label=F, group.by='ResponseInfo', pt.size = 1)
ggsave(paste0(fig_dir, 'raw.umap.ResponseInfo.pdf'), width = 7, height = 7)
DimPlot(object=merged_seu_subset, reduction='umap', label=F, pt.size=1)
ggsave(paste0(fig_dir, 'raw.umap.clusters.pdf'), width = 7, height = 7)


exp_data = merged_seu_subset@assays[["RNA"]]@scale.data[VariableFeatures(merged_seu_subset),]


SaveH5Seurat(merged_seu_subset, filename = paste0(data_dir, "seu_",sampleType_name,"_", cellType_of_interest, "_PENCIL_wrong.h5Seurat"), 
             overwrite = TRUE,keep.order=TRUE)
Convert(paste0(data_dir, "seu_",sampleType_name,"_", cellType_of_interest, "_PENCIL_wrong.h5Seurat"), dest = "h5ad", assay = "RNA", overwrite = TRUE) # .X: only scaled data for HVGs. .raw.X: all genes 
saveRDS(object = merged_seu_subset, file = paste0(data_dir, "seu_",sampleType_name,"_", cellType_of_interest,"_PENCIL_wrong.rds")) 
...
merged_seu = readRDS(file = paste0(data_dir,"seu_",sampleType_name,'_',cellType_of_interest,"_PENCIL_wrong.rds"))

exp_data_scaled = merged_seu@assays[["RNA"]]@scale.data[VariableFeatures(merged_seu),]

exp_data_unscaled = merged_seu@assays[["RNA"]]@data[VariableFeatures(merged_seu),]
write.csv(exp_data_unscaled, file=paste0(data_dir,"seu_",sampleType_name,'_',cellType_of_interest,"_PENCIL_wrong.csv"))

logTPM_counts = merged_seu@assays[["RNA"]]@counts[VariableFeatures(merged_seu),]
write.csv(logTPM_counts, file=paste0(data_dir, 'exp_data_mvg2000_log2TPM.csv'))


cell_label_df = data.frame(label = merged_seu$ResponseInfo)
write.csv(cell_label_df, file=paste0(data_dir,"cell_label_info.csv"))

emd = merged_seu@reductions[["umap"]]@cell.embeddings
...
# filter genes to only focus on protein coding genes
protein_coding_gene_file = paste0(data_dir,'/gene_with_protein_product.txt')
protein_coding_genes=read.delim(protein_coding_gene_file, header=F,colClasses = "character")[,2]
genes <- intersect(protein_coding_genes, row.names(merged_seu_subset))
seurat_obj <- seurat_obj[genes, ]
...
merged_seu$timePt <- ifelse(grepl("^Pre_", merged_seu$patient), "pre", "post")
merged_seu$patient <- gsub("^Pre_|^Post_", "", merged_seu$patient)
...
cell_counts <- cell_counts[cell_counts$PENCIL_pred_label != "Rejected", ]
...
new_df <- data.frame(meta = rep(all_patients, each = 2),
                     PENCIL_pred_label = rep(c("yes", "no"), length(all_patients)))
...
ggsave(filename = paste0(fig_dir, "stackedBar_cellRatio_", cellType_of_interest, "_",sampleType_name,".png"), height = 120, width = 190, dpi = 800, units = "mm")
...
Cells.markers = FindAllMarkers(merged_seu, only.pos=TRUE, min.pct=0.25, logfc.threshold = logFCfilter)
...
DoHeatmap(merged_seu, features = Top10.markers$gene, group.by = "PENCIL_pred_label") + NoLegend()
...
marker_sig = list(
  R_cell_marker = Top10.markers$gene[1:10],
  Rej_cell_marker = Top10.markers$gene[11:12],
  NR_cell_marker = Top10.markers$gene[13:22]
)
...
feats <- c("NR_cell_marker", "Rej_cell_marker", "R_cell_marker")
