...
# Read the .h5 file
counts <- Read10X_h5(filename = paste0(data_dir, "SKCM_GSE166181_aPD1_expression.h5"))
...
phenotype_df = read.csv(file = paste0(data_dir,"GSE166181_Metadata.tsv"), sep='\t')
phenotype_df = phenotype_df[,c(1,12,13,14)]
colnames(phenotype_df) = c('cell_ID', 'timePt', 'response', 'patient')
phenotype_df$meta <- paste(phenotype_df$patient, phenotype_df$timePt, sep = "-")

# Add phenotype info to seurat object
meta <- seurat_obj@meta.data
meta$cell_names <- rownames(meta) # save row names before merge with phenotype data
meta.new <- merge(meta, phenotype_df, by.x = "cell_names", by.y = "cell_ID")
...
#seurat_obj$timePt = NULL
#seurat_obj$response = NULL
#seurat_obj$patient = NULL


##### add original cell annotation information
cell_anno_df = read.csv(file = paste0(data_dir,"SKCM_GSE166181_aPD1_CellMetainfo_table.tsv"), sep='\t')
cell_anno_df = cell_anno_df[,c(1,6)]
