...
merged_seu$sample = merged_seu$patient
merged_seu$patient <- gsub("_.*", "", merged_seu$patient)

clinical_df_raw <- read.csv(paste0(data_dir,"clinicInfo.csv"), header = TRUE, stringsAsFactors = FALSE)
clinical_df = clinical_df_raw[c(1:4,10,11)]
colnames(clinical_df) = c("patient","sex", "age", "therapy", "OS_time", "OS_status")
meta <- merged_seu@meta.data
meta$cell_names <- rownames(meta) # save row names before merge with phenotype data
meta.new <- merge(meta, clinical_df, by.x = "patient", by.y = "patient")
rownames(meta.new) <- meta.new$cell_names # retrieve rownames 
meta.new <- meta.new[, !(names(meta.new) %in% c("cellID", "cell_names"))]
...
merged_seu$sex <- ifelse(merged_seu$sex == "M", 1, 0)
...
saveRDS(object = merged_seu, file = paste0(processed_dir, "seu_",sampleType_name,"_all_raw.rds")) 
...
merged_seu$CD8T_cell <- ifelse(grepl("CD8", merged_seu$cell_type_original, ignore.case = TRUE), "CD8+ T-cells", "Other")
